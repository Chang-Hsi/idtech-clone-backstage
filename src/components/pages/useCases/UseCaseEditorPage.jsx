...
const UseCaseEditorPage = ({ mode }) => {
  ...
  useEffect(() => {
    const loadProductOptions = async () => {
      ...
      try {
        const payload = await fetchBackstageUseCaseProductOptions('')
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const useCase = payload?.data?.useCase
        if (!useCase) throw new Error('Use case not found')

        setCurrentSlug(useCase.slug)
        setForm({
          title: useCase.title ?? '',
          subtitle: useCase.subtitle ?? '',
          description: useCase.description ?? '',
          heroImageUrl: useCase.heroImageUrl ?? '',
          status: useCase.status ?? 'active',
          linkedProductSlugs: Array.isArray(useCase.linkedProductSlugs)
            ? useCase.linkedProductSlugs
            : [],
        })

        const nextPreview = useCase.heroImageUrl ?? ''
  ...
  const syncBackgroundPreviewAfterBlur = () => {
    ...
    previewBlurTimerRef.current = setTimeout(() => {
      const nextPreview = form.heroImageUrl?.trim() ?? ''
      if (!nextPreview) {
  ...
  const submit = async (event) => {
    ...
    const validationForm = {
      ...form,
      heroImageUrl:
        imageInputMode === 'upload' && imageUploadFile && !form.heroImageUrl
          ? 'https://upload.pending.local/use-case-image.webp'
          : form.heroImageUrl,
    }
    ...
    try {
      let resolvedHeroImageUrl = String(form.heroImageUrl ?? '').trim()
      if (imageInputMode === 'upload' && imageUploadFile) {
      ...
      if (mode === 'create') {
        ...
        await updateBackstageUseCase(slug, payload)
      }

      navigate('/pages/content/use-cases', { replace: true })
    } catch (error) {
      setStatus('error')
      setErrorMessage(error.message || 'Unable to save use case')
    }
  }
      ...
      <form onSubmit={submit} className="space-y-4">
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            ...
            <ImageSourceInputField
              ...
              urlValue={form.heroImageUrl}
              onUrlChange={(nextValue) => updateField('heroImageUrl', nextValue)}
              onUrlBlur={() => {
                syncBackgroundPreviewAfterBlur()
                validateByFieldName('heroImageUrl')
              }}
              urlError={getFieldError('heroImageUrl')}
              ...
              {!(imageInputMode === 'upload' && imageUploadPreviewUrl) && !backgroundPreviewUrl ? (
                ...
                <div className="relative h-32 w-full max-w-[380px] overflow-hidden rounded-md border border-slate-200 bg-slate-50">
                  ...
                  <img
                    ...
                    alt="Use case background preview"
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Available: products not linked to this use case. Selected: products linked to this use case.
          </p>
